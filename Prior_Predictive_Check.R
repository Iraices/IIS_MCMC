...
## typical kind of expert judgment one might have
y_int <- c(-20, 80)
#y_int <- c(30, 100)
...
filterout_priors <- function(low_mu0 = -100, high_mu0 = 100,
                      ...
                      th_1 = 0.80){
val <- expand.grid(mu0  = seq(from = low_mu0, to = high_mu0, length.out = length.out),
                   tau0 = seq(from = low_tau0, to = high_tau0, length.out = length.out))
...
out <- filterout_priors(low_mu0 = -30,high_mu0 = 100,
                        low_tau0 = 5,high_tau0 = 50, 
                        y_int = y_int,length.out = 200, th_1 = th_1)
...
out <- filterout_priors(low_mu0 = -8.5, high_mu0 = 70,
                        low_tau0 = 5, high_tau0 = 17, 
                        y_int = y_int,length.out = 300, th_1 = th_1)
levelplot(p ~ mu0*tau0, data = out$val, xlab = 'mu0', ylab = 'tau0')
levelplot(dichotomic ~ mu0*tau0, data = out$val, xlab = 'mu0', ylab = 'tau0')
out$mu0_set_prior
out$tau0_set_prior
...
plot_1 <- levelplot(dichotomic ~ mu0*tau0, data = out$val, cut = 2, breaks = th_1, at = c(0,th_1,1), 
...
border_values <- data.frame(x=out$val$mu0[abs(out$val$p-th_1)<=0.01],y=out$val$tau0[abs(out$val$p-th_1)<=0.01])
...
plot_2 <- xyplot(predict(fit_model, data_order) ~ data_order$x, col = 'black', type = 'l', xlim = c(-20,100), ylim = c(4,17), 
...
plot(x = border_values_edge_df$x, y = predict(fit_model, border_values_edge_df), col = 'black',  xlim = c(-17,80), ylim = c(5,26), xlab = 'mu0', ylab = 'tau0')
par(new = TRUE)
plot(border_values_edge_df$x, border_values_edge_df$y, xlim = c(-17,80), ylim = c(5,26), xlab = 'mu0', ylab = 'tau0')
