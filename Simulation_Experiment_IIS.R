...
data_precise <- data[order(data$sigma),][1:40,]

save(random_data, file = "random_data.Rdata")
save(data_precise, file = "data_precise.Rdata")

load("random_data.Rdata")
load("data_precise.Rdata")
...
output_orig <- estimate_lower_bound (mu0 = 5, tau0 = 7, 
...
output_random_data <- estimate_lower_bound (mu0 = 5, tau0 = 7, 
                                    lower_bound_mu0 = low_mu0, upper_bound_mu0 = high_mu0, 
                                    lower_bound_tau0 = low_tau0, upper_bound_tau0 = high_tau0, 
...
initial_data_precise_time <- proc.time()

output_data_precise <- estimate_lower_bound (mu0 = 5, tau0 = 7, 
                                    lower_bound_mu0 = low_mu0, upper_bound_mu0 = high_mu0, 
                                    lower_bound_tau0 = low_tau0, upper_bound_tau0 = high_tau0, 
                                    y = data_precise$y, sigma = data_precise$sigma, y_int = y_int,
                                    n_iter_stan = n_iter_stan, region = reg, target_ESS = 5000)

final_data_precise_time <- proc.time() - initial_data_precise_time

save(output_data_precise, file = "output_data_precise.Rdata")
load("output_data_precise.Rdata")
